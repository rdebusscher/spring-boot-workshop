    ...
    public void initDependencies(ExtensionContext context) {
        if (applicationContext == null) {
            ...
            if (applicationContext instanceof AbstractApplicationContext abstractApplicationContext) {
                try {
                    // Register the IDatabaseConnection instance as a Spring Bean
                    // because this is added late to the Context, you can only use it like this in the test class.
                    // private Provider<IDatabaseConnection> databaseConnectionProvider;

                    IDatabaseConnection databaseConnection = new DatabaseDataSourceConnection(dataSource);
                    databaseConnection.getConfig().setProperty(DatabaseConfig.PROPERTY_DATATYPE_FACTORY, new CustomH2DataTypeFactory());
                    abstractApplicationContext.getBeanFactory().registerSingleton("dbunit.IDatabaseConnection", databaseConnection);
                } catch (SQLException e) {
                    throw new TestAbortedException("Error creating the IDatabaseConnection", e);
                }
            }
